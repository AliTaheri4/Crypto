...
{
    public class CandleStichDivergengeSRBackground : IInvocable

    {
        ...
        public DateTime _start { get; set; } = new DateTime(2021, 6, 8, 0, 0, 0);// new DateTime(2020, 4, 5, 0, 0, 0);// new DateTime(2020, 4, 5, 0, 0, 0);
        ...
        public CandleStichDivergengeSRBackground(ApplicationDbContext context, IDbConnection dbConnection)
        {
            _context = context;
        ...
        public async Task Execute()
        {
            ...
            while (start)
            {
                try
                {
                    ...
                    var stoch = quotesList.GetStoch(14, 3, 3).ToList();
                    var cci = quotesList.GetCci(10).ToList();
                    var mfi = quotesList.GetMfi(14).ToList();
                    var cmf = quotesList.GetCmf(21).ToList();
                   ...
                   10
                    ...
                    var checkSingnal = new CheckCandleStickDivergengeSRSingnal(_context, _dbConnection);
        ...
        private (long, long) ChangeSymbol(long from, long to)
        {
            ...
            if (LastRequestDateTime.AddMinutes((int)_timeFrame * 60 * 2) > latestDatetime)
            {
