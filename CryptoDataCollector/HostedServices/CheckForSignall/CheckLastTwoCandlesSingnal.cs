...
namespace CryptoDataCollector.CheckForSignall
{
    public class CheckLastTwoCandlesSingnal
    {
        ...
        public CheckLastTwoCandlesSingnal(ApplicationDbContext context, IDbConnection dbConnection)
        {
            _context = context;
        ...
        public SignalCheckerType CheckStrategy(List<CandleStichDivergengeSRSignalCheckingModel> list, int step, TimeFrameType timeFrame)
        {
            ...
            var mode2 = CheckForBigCandleStick();
        ...
        public SignalCheckerResult ChckForLastestCandleColors()
        {
            ...
            var lastBars = AllList.TakeLast(2).ToList();
            int count = 0;
        ...
        public SignalCheckerResult CheckForBigCandleStick()
        {

            var lastBars = AllList.TakeLast(2).ToList();

            var firstBar = lastBars.First();

            if (_TradeType == TradeType.Long)
            {
                if (SignalExtensions.GetPercentWithDiffrences(firstBar.Close, firstBar.Open) > BigCandle)
                {
                    var diffBody = SignalExtensions.GetPercentWithDiffrences(firstBar.Close, firstBar.Open);
                    if (diffBody > SignalExtensions.GetPercentWithDiffrences(firstBar.Close, firstBar.High) && diffBody > SignalExtensions.GetPercentWithDiffrences(firstBar.Open, firstBar.Low))
                    {
                        return new SignalCheckerResult() { TradeType = _TradeType, HasTriggred = true };
                    }
                }
            ...
            if (_TradeType == TradeType.Short)
            {
                ...
                {
                    var diffBody = SignalExtensions.GetPercentWithDiffrences(firstBar.Close, firstBar.Open);
                    if (diffBody > SignalExtensions.GetPercentWithDiffrences(firstBar.Close, firstBar.Low) && diffBody > SignalExtensions.GetPercentWithDiffrences(firstBar.Open, firstBar.High))
                    {
                        return new SignalCheckerResult() { TradeType = _TradeType, HasTriggred = true };
                    }
                }
        ...
        public SignalCheckerType Buy()
        {
            ...
            {
                var lastTrade = _context.Trades.Where(p => p.Symbol == _Symbol && p.TimeFrameType == _TimeFrameType && p.SignalType==_SignalType).OrderByDescending(p => p.Id).FirstOrDefault();
        ...
        public ProfitLossModel CalculateProfitLess(bool isBuyingCandle = true)
        {
            ...
            int index = 0;
            if (isBuyingCandle)
                index = AllList.Count - 3;
            else
                index = AllList.Count - 2;

            var firstBar = AllList[index];
            var diffVal = Math.Abs((firstBar.Open - LastCandle.Open) / 2);
            if (!_IsEmotional)
            {
                if (_TradeType == TradeType.Long)
                {
                    ...
                    var diff = Math.Abs(pl.Loss - LastCandle.Open);
                    pl.Profit = LastCandle.Open + Decimal.Round(diff * RR, CountDecimal, MidpointRounding.ToNegativeInfinity);//choose greater
                }
                if (_TradeType == TradeType.Short)
                {

                    pl.Loss = LastCandle.Open + Decimal.Round(diffVal, CountDecimal, MidpointRounding.ToNegativeInfinity);
                    //var diff = SignalExtensions.GetPercentWithDiffrences(pl.Loss, LastCandle.Open);
                    var diff = Math.Abs(pl.Loss - LastCandle.Open);
                    pl.Profit = LastCandle.Open - Decimal.Round(diff * RR, CountDecimal, MidpointRounding.ToPositiveInfinity);
                }
            }
            else
            {
